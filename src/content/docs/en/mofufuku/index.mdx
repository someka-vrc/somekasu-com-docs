...
- When upgrading where the major/minor version (`x` / `y` in `x.y.z`) changes, delete the existing Mofufuku folder and then import the new version.
- When upgrading from the standard edition to the PRO edition, you can overwrite if the versions are the same. (If you're unsure, delete the existing folder.)
